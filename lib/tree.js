...
async function* treeGenerator(dir, options = {}) {
    ...
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const isLast = i === files.length - 1;
            const prefix = isLast ? '└── ' + colors.RESET : '├── ' + colors.RESET;
            const gitIndicator = setGitIndicator(filename,_options.gitStatsus);
            //default ignore file
            if (_options.ignoreNodeModules && filename === 'node_modules' || filename === '.git' || _options.ignore.includes(filename)) {
                //skip
                continue;
            ...
            const textColor = setTextColor(filename, stat);
            const newFilename = gitIndicator !== '' || gitIndicator ? filename + '  ' + gitIndicator : filename;
            if (stat.isDirectory()) {
                yield colors.GREEN + _options.indent + prefix + textColor + newFilename + colors.RESET;
                const newIndent = _options.indent + (isLast ? '    ' : '│   ');
                yield* treeGenerator(fullPath, {..._options, indent: newIndent, ignoreNodeModules: _options.ignoreNodeModules, ignore: _options.ignore }); // Recurse into subdirectories
            } else {
                yield colors.GREEN + _options.indent + prefix + textColor + newFilename + colors.RESET;
            }
        }
...
module.exports = { printTree, printTreeRecursive }